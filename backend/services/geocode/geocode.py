...
class ManualOverrideGeocoder:
    ...
    def resolve(self, geocode: "Geocode", session, place: str) -> Optional[LocationOut]:
        ...
        if not hit:
            return None
        logger.info("🟧 [ManualOverrideGeocoder] Manual fix hit: %s", place)
        return LocationOut(
            raw_name=place,
...
class PermanentCacheGeocoder:
    ...
    def resolve(self, geocode: "Geocode", session, place: str) -> Optional[LocationOut]:
        ...
        try:
            ...
            logger.warning("🧯 [PermanentCacheGeocoder] Cache entry malformed for '%s': %s", key, entry)
            return None
        ...
        if lat is None or lng is None:
            return None
        logger.info("🟦 [PermanentCacheGeocoder] Cache hit for %s", place)
        return LocationOut(
            raw_name=place,
...
class HistoricalGeocoder:
    ...
    def resolve(self, geocode: "Geocode", session, place: str) -> Optional[LocationOut]:
        ...
        if not hit:
            return None
        logger.info("🟨 [HistoricalGeocoder] Historical fix hit: %s", place)
        return LocationOut(
            raw_name=place,
            normalized_name=hit.get("normalized_name", place),
            latitude=hit.get("latitude") or hit.get("lat"),
...
class FuzzyAliasGeocoder:
    ...
    def resolve(self, geocode: "Geocode", session, place: str) -> Optional[LocationOut]:
        ...
        try:
            ...
            if existing:
                logger.debug("🟢 [FuzzyAliasGeocoder] DB exact match for %s", raw)
                return LocationOut(
            ...
            for loc in db_locs:
                ...
                if sim >= 90 and loc.latitude and loc.longitude:
                    logger.info("🟪 [FuzzyAliasGeocoder] DB fuzzy match for '%s' ≈ '%s' (%.1f%%)", raw, compare_to, sim)
                    return LocationOut(
                        raw_name=raw,
            ...
            logger.exception("❌ [FuzzyAliasGeocoder] DB lookup failed for %s", raw)
        return None
...
class ExternalAPIGeocoder:
    ...
    def resolve(self, geocode: "Geocode", session, place: str) -> Optional[LocationOut]:
        ...
        logger.info("✅ [ExternalAPIGeocoder] %s geocode for '%s'", src, place)
        return LocationOut(
            raw_name=place,
            normalized_name=norm,
            latitude=lat,
...
class Geocode:
    ...
    def _load_cache(self) -> Dict[str, Any]:
        if self.cache_file.exists():
            try:
                ...
                logger.warning("⚠️ [Geocode] Cache corrupted — backing up and starting fresh.")
                # Backup old corrupted cache before nuking
                backup_path = self.cache_file.with_suffix(".bak")
                self.cache_file.rename(backup_path)
                logger.info("🗃️ [Geocode] Corrupted cache backed up to %s", backup_path)
        return {}
    ...
    def _save_cache(self) -> None:
        if self.cache_enabled:
            try:
                ...
                logger.error("❌ [Geocode] Could not save cache: %s", e)
    ...
    def _retry(self, fn, *args, retries=2, backoff=1, **kwargs):
        for attempt in range(1, retries + 1):
            try:
                ...
                logger.warning("⚠️ [Geocode] Request fail (%d/%d): %s", attempt, retries, e)
    ...
    def get_or_create_location(self, session, place: str) -> Optional[LocationOut]:
        ...
        now = time.time()
        logger.debug("🔍 [Geocode] geocode chain for: %s", raw)

        for plugin in self.plugins:
            try:
                result = plugin.resolve(self, session, raw)
            except Exception as e:
                logger.exception("❌ [Geocode] Plugin %s failed for '%s': %s", plugin.__class__.__name__, raw, e)
                continue
            if result:
                if self.cache_enabled and not isinstance(plugin, PermanentCacheGeocoder):
                    ...
                    self._save_cache()
                logger.info("✅ [Geocode] %s plugin resolved '%s'", plugin.__class__.__name__, raw)
                return result
        ...
        logger.warning("❌ [Geocode] Geocode miss for raw='%s', normalized='%s'", raw, key)
        return None
