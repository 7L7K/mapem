...
Revision ID: 0e5caa1cc23d
Revises: 
Create Date: 2025-05-22 12:16:58.604785
...
def upgrade() -> None:
    ...
    sa.Column('status', sa.Enum('valid', 'vague_state', 'unresolved', 'historical', 'duplicate', 'manual_override', 'ok', name='location_status_enum'), nullable=False),
    ...
    op.create_index(op.f('ix_tree_uploaded_tree_id'), 'tree', ['uploaded_tree_id'], unique=False)
    ...
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.ForeignKeyConstraint(['individual_id'], ['individuals.id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['tree_version_id'], ['tree.id'], ondelete='CASCADE'),
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_tree_uploaded_tree_id'), table_name='tree')
    op.drop_table('tree')
